...
type Props = {
  task: Task;
  switchItemStatus: (arg: string) => void;
  deleteTask: (arg: string) => void;
};

const TaskItem = ({ task, switchItemStatus, deleteTask }: Props) => {
  return (
    <div className={styles.task}>
      <div className={styles.taskContent}>
        <div
          ...
          onClick={() => {
            switchItemStatus(task.id);
          }}>
          <div className={task.isDone ? styles.checkboxDone : styles.checkbox}>
            {task.isDone ? (
              <svg
                width="46"
                height="46"
          ...
          <div className={task.isDone ? styles.nameCompleted : styles.name}>
            {task.name}
          </div>
        </div>
